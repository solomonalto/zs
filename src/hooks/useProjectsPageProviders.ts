...
export function useProjectsPageProviders() {
  ...
  const createProvider = useCallback(
    async (provider: any) => {
      ...
      try {
        // Remove avatar_file from provider object before saving to database
        const { avatar_file, ...providerData } = provider;

        // Save with null avatar_url - will be uploaded on publish
        const { data, error: err } = await supabase
          .from('projects_page_providers')
          .insert([
            {
        ...
        // Store pending avatar file for later upload on publish
        if (avatar_file && avatar_file instanceof File) {
          pendingAvatarsRef.current[data.id] = avatar_file;
        }
