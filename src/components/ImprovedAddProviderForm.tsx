...
export default function ImprovedAddProviderForm({
  ...
  const [startingRateCurrency, setStartingRateCurrency] = useState('UGX');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const provider: any = {
      ...
      starting_rate_currency: startingRateCurrency,
    };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Starting Rate with Currency (All Types) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">Starting Rate</label>
          <div className="flex gap-2">
            <select
              value={startingRateCurrency}
              onChange={(e) => setStartingRateCurrency(e.target.value)}
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-3 glass-effect rounded-xl border text-white bg-transparent focus:ring-2 focus:ring-rose-400 focus:border-transparent transition-all ${
